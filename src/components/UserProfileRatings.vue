    ...
    <div class="col-md-9 col-sm-12 col-xs-12">
        ...
        <div class="row ipad-width2">
            ...
            <div class="topbar-filter">
                <p>Found <span>{{ totalElements }} rates</span> in total</p>
            </div>
            <div v-for="(rating,index) in userRatings" :key="index" class="movie-item-style-2 userrate">
                <img
                    v-if="rating.movie.posterImage != null"
                    :src="
                        $MOVIE_PHOTOS_URL + '/' + rating.movie.posterImage
                    "
                    alt=""
                    />
                    <img
                ...
                <div class="mv-item-infor">
                    <h6><a href="#">{{ rating.movie.title }} </a></h6>
                    <p class="time sm-text">your rate:</p>
                    <p class="rate"><i class="ion-android-star"></i><span>{{ rating.userRating }}</span> /5</p>
                    <p class="time sm">{{
...
export default {
    ...
    methods: {
        async getUserRatings(page, size) {
            const response = await UserService.getUserRatings(this.userId, page, size);
